...
| Follower stops logging with no error | FH6 lost foreground (it only drives when focused) | re-focus the game (`refocus_forza.ps1` in the forza-computer-use skill); the watchdog's no-growth check restarts it |
| Recovery loops "Controller-Disconnected dialog -> A + Enter" forever | the pad the game thinks is gone cannot prove it is back by pressing buttons | since 08-01 recovery RE-PLUGS the vpad after 4 failed taps (`replug_pad()`), which raises a fresh connection event and clears it. If clearing by hand, create a NEW pad rather than pressing harder |
| `_csv.Error: field larger than field limit` reading follow_log.csv | TWO followers writing the same log, interleaved | kill all followers (and the watchdog first), archive the corrupt log, restart one. Never hand-launch a follower while the watchdog is up |
