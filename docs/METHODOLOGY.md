...
1. **Never A/B over a live learner.** Freeze vtrim for scored windows (`vtrim_up/dn/cut/netscale=0`
   **with `vtrim_on=1`** - `vtrim_on=0` removes the map's boost instead of freezing it), or
...
3. **Calibrate thresholds on the current config before using them as aborts.** Healthy production
   ...
   good rung **(measured)**.
4. **Abort monitors must ACT** - write the rollback key themselves, never just print a verdict
...
- **Feedforward saturates** - `ffm_w` 0.15 optimal, 0.30 strictly worse; reactive gains must
  shed as FF grows (`ffm_gsc`) **(measured)**.
- **Full stick = front tires at their limit** (game speed-sensitive steering); `fc_frac` is
  total grip and stays submaximal in understeer - "add steering authority" framings are wrong.
