...
echo "== ARM 4 (combined): every arm that won, plus pdg_gain =="
echo "   pdg_gain alone is ~0.03-0.15 s, at/below the 0.09 s resolution of a 40-min window,"
echo "   so it is never worth a solo window; it rides along with the winners instead."
echo "   EDIT the --arm below to include only the keys that actually won above, then rerun."
echo "   e.g. --arm '{\"pdg_gain\":2.0,\"slip_target\":1.35}'"
